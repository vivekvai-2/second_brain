...
### 🔴 Group 2 sourcing run — 2026-08-15. A vault claim refuted, and a candidate pattern.

**⚠#322 closed — and primary sourcing refuted the claim it was flagging.** [[07_Institutions/Corpus/NPCI/NPCI_1_AI_ML_Payments_Operations_2025_2026]] created from three NPCI primary documents. **"NPCI runs federated AI fraud scoring across UPI" — carried in [[05_Concepts/05_Concepts_Signature/Governance_Debt]] since 2 August and inherited by Decision_Infrastructure's Three-Instance Gate — is not supported by any NPCI source.** No document describes federated learning; the AI/ML passage in NPCI's fraud Best Practices addresses **member banks**, not EFRM; the only NPCI AI system named live or piloted is the **UPI Help Assistant**, a grievance-resolution SLM.

**The replacement finding is stronger than the one it replaces.** NPCI's EFRM issues **ecosystem-level fraud alerts member banks are directed to prioritise** — a decision input crossing an institutional boundary — while NPCI sits **inside** a binding cyber-resilience perimeter (`RBI/DPSS/2024-25/123`, Large Non-Bank PSO, from 1 Apr 2025) and **outside every AI-governance perimeter**, because RBI's AI instruments bind Regulated Entities and NPCI is a PSO, not an RE. The 2021 Digital Payment Security Controls Directions do not reach it either. **Same regulator, same statutory power, same entity — obligations imposed for cyber, withheld for AI. That is Deontic-Placement Debt, not absence.** And the restated instance no longer depends on an unverified technical characterisation.

**⚠#320 only partially closed.** Microsoft **Project Perception** (27 Jul 2026) is the taxonomy's primary source — which also settles attribution: the carousel is **downstream of Microsoft** by three weeks, so the six-layer stack must be cited to Microsoft rather than claimed as original. Two reasons it is not closed: the layer names were confirmed partly from third-party blogs against a prompt that **supplied the names in advance** (⚠#366), and the governance material appears to conflate Project Perception's "Harness" layer with the separate **Microsoft Agent Framework** harness — different products, shared word (⚠#367). **No Format B node written; fetch the Microsoft URLs directly first.**

**A candidate pattern, two instances in one day.** If Microsoft ships approval gates, RBAC-controlled workflow modification and replayable orchestration telemetry as product defaults, then orchestration governance is **generally available and required by nobody** — the capability objection dies and the gap becomes purely deontic. That is structurally identical to the ISO 42001 finding recorded hours earlier: India's complete assurance chain with no regulator attaching consequence. Provisional name **supply-side governance surplus**, recorded at Orchestration_Governance and Assurance_Reuse_Gap. **Two instances is not a gate — ⚠#368 bars promotion until a third, independently sourced instance exists.**

**New flags ⚠#361–#368.** New node candidate: `RBI/DPSS/2024-25/123`, the only instrument reaching NPCI as operator, absent from the vault.
