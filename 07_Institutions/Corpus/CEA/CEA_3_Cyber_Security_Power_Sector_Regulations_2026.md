...
### S5.7 — ISO 27001 is mandated **but not in force**: a correction, itself corrected

> 🔴 **CORRECTION 2026-08-15.** This section was written on 2026-08-09 reasoning from reg 5(24) as an operative mandate. **Reg 5(24) is one of the six provisions deferred by reg 1(2)** — a fact this node records correctly at Commencement, at RU6, and at ⚠#312, and then contradicted here. No Authority order bringing it into force had issued as at 15 August 2026. Verified against the CEA-hosted notified PDF (see Log). The section below is retained with its claim narrowed: India has **enacted and not commenced** a management-system certification mandate. Downstream correction applied at [[06_Frameworks/ISO_IEC_42001]]. Raised as **⚠#357** — a node applying its own deferral warning inconsistently is a class of error worth watching for elsewhere in the corpus.
...
- **✅ ⚠#310 — SUBSTANTIALLY CLOSED 2026-08-15, not fully.** The null finding was re-tested against the **CEA-hosted notified PDF** at `cea.nic.in/wp-content/uploads/notification/2026/08/Cyber_Regulations_Notification.pdf` — a different and better source than the Gazette scan originally used. **The null holds:** no occurrence of *artificial intelligence*, *AI*, *machine learning*, *algorithm*, *automated*, *model* or *autonomous* anywhere in the regulations, definitions or Schedules. Two independent extractions of two different source files now agree. **Residual:** this second pass was a document-reading pass, not a programmatic string sweep of extracted text, and the CEA-hosted file is also bilingual rather than English-only. For a headline published claim, run a programmatic sweep on extracted text. **The reputational risk is now low enough to cite the null finding in draft work.** ⚠ **A separate ⚠#358 is raised by the same check — see below.**
- **⚠#358 (MEDIUM) — NEW.** The re-check reports **35 definitions in reg 3**; this node states **40** in two places (Chapter I table, and the term-sweep sentence). One count is wrong and the discrepancy is unresolved. **Count the definitions against the primary text before citing either figure.** Not a substantive finding either way, but a figure stated twice in a node is exactly the kind of detail that reaches a manuscript.
- **⚠#359 (LOW) — NEW.** This node cites the certificate at reg 3(1)(jj) as **"Technical Criteria Certificate"**; the re-check renders reg 5(24) as **"ISO/IEC 27001 certificate or technical standard certificate"**. Confirm the defined term against the primary text.
- **~~⚠ #310 (MEDIUM).~~** *(original text retained)* The AI null finding rests on a term sweep of the extracted English text. Given the extraction quality caveat above, re-run the sweep against a clean CEA-hosted PDF before publishing the null finding as a headline claim. The finding is very likely robust — no AI-adjacent vocabulary appears anywhere in the 40 definitions or the 33-component Cyber Security Policy specification, where it would necessarily surface — but it is the node's principal contribution and warrants a clean confirmation.
...
- 2026-08-15: **⚠#310 re-tested and substantially closed.** Perplexity pass P-02 **failed** — it returned the **2024 draft** (`/notification/2024/08/Draft_CEA_..._2024_English_Version.pdf`) plus a vendor whitepaper, not the notified 2026 instrument, and its term sweep is therefore of the wrong document and carries no weight. The notified text was instead located directly at `cea.nic.in/wp-content/uploads/notification/2026/08/Cyber_Regulations_Notification.pdf` and read in-session. Confirmed from that source: title, notification date 31 July 2026, Gazette No. 484, 17 regulations, 2 Schedules, the AI null, reg 5(24) text, and reg 1(2)'s deferral of 5(9)/5(24)/5(33)/5(39)/6(2)/6(7). Discrepancies raised as ⚠#358 (definitions count) and ⚠#359 (certificate name). S5.7 corrected for the deferral (⚠#357), with the downstream correction applied to ISO_IEC_42001.
