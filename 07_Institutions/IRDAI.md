---
type: Institution
tags:
  - type/institution
  - status/active
  - paper/p3-primary
  - paper/p4-adjacent
  - paper/fsqca
  - engine/phd
  - engine/consulting
---

# IRDAI

**Type:** Institution **Status:** Active **Last Updated:** 2026-07-14

---

## Core Synthesis

The Insurance Regulatory and Development Authority of India is the insurance-sector complement to RBI and SEBI in the PRIS Regulatory Parallelism diagnosis. IRDAI's AI governance posture as of Batch 15 continuation is predominantly indirect — the Maintenance and Sharing of Information Regulations, 2025 (IRDAI_1) is a data-governance/information-maintenance framework with **no explicit AI/ML provisions**, positioning IRDAI as a *fourth silo* in India's fragmented AI-governance architecture (RBI + SEBI + MeitY + IRDAI) despite substantive AI adoption in underwriting, claims, and fraud detection across the insurance sector. IRDAI's absence from AI-specific rulemaking, combined with its statutory mandate over an increasingly AI-intensive sector, generates the strongest "Regulatory Parallelism through omission" evidence in the P3 corpus.

**Update, 2026-08-02 — the picture is now two-track, not zero-track.** IRDAI constituted a Working Group on AI (WG-AI) on 17 June 2026, tasked with recommending an AI governance framework by ~18 September 2026 — and, unusually, seated a serving CERT-In scientist and the former CEO of RBI's technology subsidiary (ReBIT) alongside industry CISOs, making this the corpus's strongest positive cross-regulator personnel-coordination instance despite IRDAI being its weakest substantive-output institution. IRDAI's InsurTech Regulatory Sandbox has also processed 350+ applications across two cohorts, though without disaggregating AI-specific products. Neither track has yet produced a binding AI-specific obligation. Full detail, plus international comparators (NAIC, EIOPA, IAIS), now live at [[03_Domains/Insurance_AI_Governance]] — this institution note's "fourth silo, zero movement" framing should be read alongside that domain's fuller "study-in-progress, not silence" picture.

## Institutional Profile

- **Type:** Statutory insurance regulator
- **Established:** 1999 (statutory authority under IRDA Act, 1999)
- **Governing statute:** Insurance Regulatory and Development Authority Act 1999; Insurance Act 1938 (sections including S.20 cited as basis for public-interest information sharing)
- **Primary mandate:** Insurance sector regulation, policyholder protection, market development
- **AI-relevant jurisdiction:** Insurance companies (life, general, health, reinsurers), brokers, agents, third-party administrators, insurtech intermediaries; Insurtech Regulatory Sandbox (IRS) operational since 2019
- **Enforcement mechanism:** Registration/renewal control, monetary penalties, prohibition orders, licence suspension/cancellation

## Key AI Governance Instruments (PRIS-relevant)

| Instrument | Date | Role in PRIS |
|---|---|---|
| Maintenance and Sharing of Information Regulations, 2025 (IRDAI_1) | 2025 (exact gazette date ⚠VERIFY) | S2/S5 secondary-source corpus node; data governance without AI/ML explicit provisions; "Regulatory Parallelism through omission" evidence |
| Working Group on AI (WG-AI) constitution (IRDAI_2) | 17 June 2026 (Ref IRDAI/GA&HR/ORD/MISC/90/06/2026) | Corpus's strongest positive Institutional_Coherence instance (CERT-In + ReBIT-adjacent personnel); recommendations due ~18 Sep 2026 — see [[03_Domains/Insurance_AI_Governance]] |
| Fraud Monitoring Framework Guidelines, 2025 (IRDAI_3) | 9 Oct 2025, effective 1 Apr 2026 | Technology-neutral fraud-governance instrument, de facto AI-relevant given fraud-detection's AI/ML implementation in practice |
| Regulatory Sandbox Regulations, 2025 (IRDAI_4) | 2025 | InsurTech experimentation pathway, 350+ applications across 2 cohorts, not AI-disaggregated |

## Institutional Logic (P3/P4 Analytical Relevance)

IRDAI's governing logic is **policyholder protection and market conduct in an information-asymmetric market** — the classical insurance-regulator orientation focused on solvency, claims-handling fairness, and mis-selling prevention. In contrast to RBI's prudential-risk orientation and SEBI's market-conduct orientation, IRDAI's AI governance would logically emphasise underwriting fairness (non-discrimination), claims decisioning transparency, and fraud detection accountability. As of July 2026, no such AI-specific instrument has been issued. This produces the P3/P4 finding that India's fourth-silo AI governance gap is at IRDAI: the insurance sector is a high-AI-adoption context with the weakest sectoral AI-governance overlay. The vendor and data-localisation provisions in IRDAI_1 create partial data-governance coverage but do not address AI-specific risks (model bias in underwriting, algorithmic claims-denial, fraud-detection false positives).

## Consulting & Outreach Relevance

- Partner gate: Publication (P3/P1) then consulting engagement
- Value proposition to IRDAI: AI-specific regulatory design assistance; comparative analysis of BFSI regulator AI instruments (RBI FREE-AI + SEBI Reg. 16C + international MAS/IAIS) as reference architecture for an IRDAI AI framework; insurtech sandbox extension to AI-specific evaluation gates
- Engagement pathway: IRDAI consultation responses (any forthcoming AI framework); insurance-industry association (Life Insurance Council, General Insurance Council) advisory work; academic + practitioner insurance AI ethics forums

## Linked Projects

- [[P3_BFSI_JEIM]]
- [[P4_Doctrinal_IJLIT]]
- [[fsQCA_Thesis_Chapter]]

## Linked Concepts

- [[Regulatory_Parallelism]]
- [[Governance_Debt]]
- [[Institutional_Coherence]]
- [[Governance_Capacity]]

## Linked Domains

- [[BFSI_Governance]]
- [[AI_Governance]]
- [[03_Domains/Insurance_AI_Governance]] (added 2026-08-02 — primary domain for IRDAI's AI-specific tracking)

## Linked Institutions

- [[RBI]]
- [[SEBI]]
- [[MEITY]]
- [[CERT_IN]]

## Notes

- **⚠ VERIFY (from IRDAI_1):** Exact gazette notification date/number for the Maintenance and Sharing of Information Regulations, 2025 not confirmed from primary source; IRDAI_1 corpus node is built from a secondary consultancy commentary — primary regulatory text (IRDAI document portal documentId=6540652) has not been retrieved.
- **⚠ VERIFY:** Whether IRDAI has published any AI-specific circular, discussion paper, or consultation document between 2024 and July 2026 addressing algorithmic underwriting, GenAI in claims processing, or model-risk requirements for insurers.

---

_Back to [[_Institutions_MOC]]_
