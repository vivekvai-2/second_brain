...
- **🟡 ⚠#320 — PARTIALLY CLOSED 2026-08-15. Do not treat as closed.** Pass P-04 identified a primary Microsoft source: **Project Perception**, announced in *"Rethinking security for the age of AI"* (blogs.microsoft.com, **27 July 2026**, Hayete Gallot, EVP Microsoft Security; public preview 3 Aug 2026), with an architecture page at `microsoft.com/en-us/security/business/ai-powered-cybersecurity/project-perception-agentic-system`. Microsoft's six layers reportedly read **Signals & sensors · Security context · Models · Harness · Agents · Actuators** — matching the carousel's naming closely enough that the carousel is almost certainly **downstream of the Microsoft launch** (27 July, three weeks before the carousel was triaged on 9 August). **That resolves the attribution question: the taxonomy is Microsoft's, not original to this vault, and must be cited as such.**
  **Why it is not fully closed — ⚠#366 (MEDIUM).** The layer names are confirmed only from a mix of Microsoft pages and third-party blogs (`flowith.io`, `fourweekmba.com`), and the retrieval was run against a prompt that *supplied the six layer names in advance* — a confirmation-shaped query. **Fetch the two Microsoft URLs directly and confirm the six layers verbatim from Microsoft's own text before writing the Format B node.**
  **⚠#367 (MEDIUM) — a probable conflation, and it touches the construct's core.** The governance material P-04 returned (approval-policy engine, PIM/RBAC control over who may alter agent roles, full reconstructability of orchestration decisions in Sentinel/Purview) is sourced substantially to `learn.microsoft.com/en-us/agent-framework/agents/harness` — **the Microsoft Agent Framework, a developer product, whose "harness" is not established to be the same thing as Project Perception's "Harness" layer.** The word is shared; the referent may not be. **Do not attribute Agent Framework governance controls to Project Perception without a Microsoft source that joins them.**
- **~~⚠ #320 (MEDIUM).~~** *(original)* The Microsoft AI-native security reference architecture (Signals → Context → Models → Harness → Agents → Actuators) is the construct's originating source and **is not in the corpus as a node.** The six-layer taxonomy and the "humans remain in control" statement are quoted from the practitioner carousel, not from a primary Microsoft document. **Create a Wiki Intake node for the Microsoft architecture before citing the layer taxonomy in publication-track output.**
...
## 🔷 The vendor is building what no regulator has asked for (2026-08-15)

Provisional, pending ⚠#366/#367 — but if Microsoft's harness does what P-04 reports, it changes what this construct can claim, and sharpens rather than weakens it.

Microsoft is reported to implement, at the orchestration layer: an **approval-policy engine** gating state-changing actuators behind human authorisation with administrator-scoped allowances; **Entra PIM / Defender RBAC** control over who may alter agent roles, bind new MCP tool servers, adjust execution budgets or change routing handoffs; and **structured telemetry** logging the model that handled each sub-task, the retrieved context window, the prompt chain, tool-invocation parameters and actuator execution status — for post-incident replay.

**Those are precisely the three questions this construct says nobody governs:** who approved the workflows the harness may run, who can change them and under whose authority, and whether an orchestration decision can be reconstructed months later.

**The claim does not collapse — it relocates.** This note's argument was never that the capability is impossible; it was that **no regulator requires it**. If a hyperscaler ships approval gates, RBAC-controlled workflow modification and replayable orchestration telemetry as product features in July 2026, then:

1. **The capability objection is dead.** No regulator can say orchestration governance is technically premature — it is generally available.
2. **The gap is now purely deontic.** SR 26-2 carves agentic AI out of model-risk scope; India has not reached the question; and the controls exist and ship by default. That is a much harder position for a regulator to defend than "the technology isn't ready."
3. **A new asymmetry appears — vendor-supplied governance without regulatory demand.** Controls are configured by tenant administrators at their own discretion, with no external standard for what "adequate" means, no obligation to enable them, and no supervisory visibility into whether they are on. **Governance capability without governance obligation.**

**This is the second instance of the same structure recorded on 2026-08-15**, alongside [[06_Frameworks/ISO_IEC_42001]]'s *assurance before demand* — a complete assurance chain in India with no regulator attaching consequence to its output. Two sightings in one day, in different layers (certification; runtime controls) and different geographies, is worth treating as a candidate pattern rather than a coincidence. Provisional name: **supply-side governance surplus**. Cross-recorded at [[05_Concepts/05_Concepts_Standard/Assurance_Reuse_Gap]].

⚠#368 — two instances is not a Three-Instance Gate. Do not promote to a named construct until a third, independently sourced instance exists.
